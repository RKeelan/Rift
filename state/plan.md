...
## Task 2: Agent adapters and session manager

### Requirements

Define the message protocol in `shared/`, implement the echo adapter, and build session management with REST endpoints. The `ClaudeCodeAdapter` (for live agent integration) is deferred to a separate request since its stdin/stdout protocol has not been verified.

**Message protocol** — defined in `shared/src/types.ts`:
- `ClientMessage`: `{ type: "user_message", content: string }`
- `ServerMessage`: a discriminated union:
  - `{ type: "assistant_text", content: string }` — text from the agent (may arrive as multiple messages that the client concatenates)
  - `{ type: "tool_use", id: string, tool: string, input: object }` — agent is calling a tool (emitted only after the full tool input has been buffered)
  - `{ type: "tool_result", id: string, output: string, is_error: boolean }` — tool result
  - `{ type: "session_event", event: "started" | "stopped" | "error", message?: string }` — lifecycle events
  - `{ type: "history", messages: ServerMessage[] }` — replay buffer on reconnect

**Agent adapters** — defined in `server/src/adapters/`:
- `AgentAdapter` interface: `spawn(config): Promise<void>`, `send(message: string): void`, `onMessage(cb: (msg: ServerMessage) => void): void`, `onExit(cb: (code: number, error?: string) => void): void`, `stop(): void`. Contracts: `spawn()` rejects if the adapter fails to start; `send()` before `spawn()` throws; `send()` after `stop()` is silently ignored; `stop()` on an already-stopped adapter is a no-op. `onExit` fires when the underlying process terminates on its own (normal exit or crash); the `SessionManager` uses this to transition the session to `stopped`
- `EchoAdapter`: for any input string, emits the following sequence: (1) an `assistant_text` message echoing the input prefixed with "Echo: ", (2) a `tool_use` message with `tool: "Read"`, `id: "<uuid>"`, and `input: { file_path: "example.ts" }`, (3) a `tool_result` message with the same `id`, `is_error: false`, and `output` set to exactly:
  ```
  --- a/example.ts
  +++ b/example.ts
  @@ -1,3 +1,3 @@
  ...
  +const y = 3;
   const z = x + y;
  ```
  This deterministic sequence and exact diff format enable validation of Markdown rendering, tool-card rendering, and diff rendering in Tasks 3–6. This is the only adapter implemented in this plan; live agent support is deferred

**Session manager** — `server/src/session.ts`:
- `SessionManager` class: creates sessions (each wraps an adapter), assigns UUIDs, buffers all `ServerMessage`s in memory, tracks state (`running` | `stopped`)
- Buffer limit: cap at 10,000 messages per session. When exceeded, drop the oldest messages and log a warning
- Stopped sessions are cleaned up after 1 hour (server-side TTL) via a `setInterval` check every 5 minutes (injectable for testing with fake timers)
- Graceful shutdown: on SIGTERM/SIGINT, stop all running sessions (which stops adapter processes), then exit
- REST endpoints:
  - `POST /api/sessions` — create a session (accepts `workingDirectory`; defaults to `WORKING_DIR` env var). If the adapter fails to spawn, return 500 with the standard error envelope. If the adapter process dies shortly after spawn, the session transitions to `stopped` with an error message via the `onExit` callback
  - `GET /api/sessions` — list sessions (id, state, creation time)
  - `GET /api/sessions/:id` — return a single session's details (id, state, creation time). Returns 404 if not found
  - `DELETE /api/sessions/:id` — stop a session

Note: sessions have a per-session `workingDirectory` that affects the agent process. The file browser (Task 7) and git endpoints (Tasks 8–9) use the global `WORKING_DIR` environment variable, not the per-session directory. These views show the state of a single project directory, independent of which sessions exist

### Verification

- Unit tests for `EchoAdapter`: spawn, send, receive expected `ServerMessage` sequence
- Unit tests for `SessionManager`: create, list, stop, message buffering, retrieving buffer, buffer eviction at limit, TTL cleanup of stopped sessions
- Integration test with supertest: create session via REST, verify it appears in list, fetch by id, stop it, verify state changes
- Integration test: fetch non-existent session id, verify 404
- Integration test: simulate adapter spawn failure, verify `POST /api/sessions` returns 500

### Validation

- Start server (defaults to echo adapter)
- `curl -X POST http://localhost:3000/api/sessions -d '{"workingDirectory":"/tmp"}' -H 'Content-Type: application/json'` — returns session id
- `curl http://localhost:3000/api/sessions` — shows the session as `running`
- `curl -X DELETE http://localhost:3000/api/sessions/<id>` — stops it
- `curl http://localhost:3000/api/sessions` — shows the session as `stopped`
